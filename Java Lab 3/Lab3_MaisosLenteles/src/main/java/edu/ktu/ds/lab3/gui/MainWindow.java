...
public class MainWindow extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private final ComboBox<String> cmbCollisionTypes = new ComboBox<>();
    private final ComboBox<String> cmbHashFunctions = new ComboBox<>();
    ...
    private void initComponents() {
        ...
        paneButtons = new Panels(
                new String[]{
                        ...
                        MESSAGES.getString("button3")}, 1, 4);
        paneButtons.getButtons().forEach((btn) -> btn.setOnAction(this));
        IntStream.of(1, 2).forEach(p -> paneButtons.getButtons().get(p).setDisable(true));
    ...
    private void mapGeneration(String filePath) {
        ...
        if (filePath == null) {
            ...
            IntStream.of(1, 2).forEach(p -> paneButtons.getButtons().get(p).setDisable(false));
        ...
        String[][] modelList = map.getMapModel(paneParam1.getTfOfTable().get(5).getText());
